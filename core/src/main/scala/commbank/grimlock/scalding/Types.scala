...
/**
 * Rich wrapper around a `TypedPipe[(Position[P], Type)]`.
 *
 * @param data `TypedPipe[(Position[P], Type)]`.
 *
 * @note This class represents the variable type along the dimensions of a matrix.
 */
case class Types[P <: Nat](data: TypedPipe[(Position[P], Type)]) extends FwTypes[P] with Persist[(Position[P], Type)] {
