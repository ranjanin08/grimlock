...
class TestHashSampleToSize extends TestSample {
  ...
  "A HashSampleToSize" should "select 25% correctly" in {
    ...
    (1 to 10000).map { case i => if (obj.select(toCell(Position2D(2 * i, i)), ext)) 1 else 0 }.sum shouldBe 2500 +- 50
  }
  ...
  it should "select 50% correctly" in {
    ...
    (1 to 10000).map { case i => if (obj.select(toCell(Position2D(2 * i, i)), ext)) 1 else 0 }.sum shouldBe 5000 +- 50
  }
  ...
  it should "select 75% correctly" in {
    ...
    (1 to 10000).map { case i => if (obj.select(toCell(Position2D(2 * i, i)), ext)) 1 else 0 }.sum shouldBe 7500 +- 50
  }
}
...
class TestAndThenSampler extends TestSample {
  ...
  "A AndThenSampler" should "select correctly" in {
    ...
    val ext = Map(Position1D(Second.toString) -> Content(DiscreteSchema[Codex.LongCodex](), 10000))
    val res = (1 to 10000).flatMap { case i => if (obj.select(toCell(Position2D(i, i)), ext)) Some((i,i)) else None }
