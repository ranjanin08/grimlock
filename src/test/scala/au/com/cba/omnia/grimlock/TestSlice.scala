...
class TestOverPosition1D extends FlatSpec with Matchers with TestSlicePosition1D {

  val over = Over[Position1D, First.type](First)

  "A Over[Position1D]" should "return a Position1D for the selected dimension" in {
    over.selected(pos1) should be (Position1D(pos1(First)))
  }
  ...
  it should "return a map" in {
    over.toMap(Cell(pos1, con1)) should be (Map(over.selected(pos1) -> con1))
  }

  it should "combine two maps" in {
    over.combineMaps(pos1, over.toMap(Cell(pos1, con1)), over.toMap(Cell(pos2, con2))) should
      be (Map(over.selected(pos1) -> con1, over.selected(pos2) -> con2))
    over.combineMaps(pos1, over.toMap(Cell(pos1, con1)), over.toMap(Cell(pos1, con1))) should
      be (Map(over.selected(pos1) -> con1))
  }
}

class TestAlongPosition1D extends FlatSpec with Matchers with TestSlicePosition1D {

  val along = Along[Position1D, First.type](First)

  "A Along[Position1D]" should "return a Position0D for the selected dimension" in {
    along.selected(pos1) should be (pos1.remove(First))
  }
...
class TestOverPosition2D extends FlatSpec with Matchers with TestSlicePosition2D {

  val list = List((Over[Position2D, First.type](First), First),
    (Over[Position2D, Second.type](Second), Second))

  "A Over[Position2D]" should "return a Position1D for the selected dimension" in {
    list.map { case (o, d) => o.selected(pos1) should be (Position1D(pos1(d))) }
  }
  ...
  it should "combine two maps" in {
    list.map {
      case (o, _) =>
        o.combineMaps(pos1, o.toMap(Cell(pos1, con1)), o.toMap(Cell(pos2, con2))) should
          be (Map(o.selected(pos1) -> Map(o.remainder(pos1) -> con1),
            o.selected(pos2) -> Map(o.remainder(pos2) -> con2)))
        o.combineMaps(pos1, o.toMap(Cell(pos1, con1)), o.toMap(Cell(pos1, con1))) should
          be (Map(o.selected(pos1) -> Map(o.remainder(pos1) -> con1)))
    }
  }
}

class TestAlongPosition2D extends FlatSpec with Matchers with TestSlicePosition2D {

  val list = List((Along[Position2D, First.type](First), First),
    (Along[Position2D, Second.type](Second), Second))

  "A Along[Position2D]" should "return a Position1D for the selected dimension" in {
    list.map { case (a, d) => a.selected(pos1) should be (pos1.remove(d)) }
  }
  ...
  it should "return a map" in {
    list.map {
      case (a, _) => a.toMap(Cell(pos1, con1)) should be (Map(a.selected(pos1) -> Map(a.remainder(pos1) -> con1)))
    }
  }
  ...
  it should "combine two maps" in {
    list.map {
      case (a, _) =>
        a.combineMaps(pos1, a.toMap(Cell(pos1, con1)), a.toMap(Cell(pos2, con2))) should
          be (Map(a.selected(pos1) -> Map(a.remainder(pos1) -> con1),
            a.selected(pos2) -> Map(a.remainder(pos2) -> con2)))
        a.combineMaps(pos1, a.toMap(Cell(pos1, con1)), a.toMap(Cell(pos1, con1))) should
          be (Map(a.selected(pos1) -> Map(a.remainder(pos1) -> con1)))
    }
  }
}
...
class TestOverPosition3D extends FlatSpec with Matchers with TestSlicePosition3D {

  val list = List((Over[Position3D, First.type](First), First),
    (Over[Position3D, Second.type](Second), Second),
...
class TestAlongPosition3D extends FlatSpec with Matchers with TestSlicePosition3D {

  val list = List((Along[Position3D, First.type](First), First),
    (Along[Position3D, Second.type](Second), Second),
...
class TestOverPosition4D extends FlatSpec with Matchers with TestSlicePosition4D {

  val list = List((Over[Position4D, First.type](First), First),
    (Over[Position4D, Second.type](Second), Second),
    (Over[Position4D, Third.type](Third), Third),
    (Over[Position4D, Fourth.type](Fourth), Fourth))
...
class TestAlongPosition4D extends FlatSpec with Matchers with TestSlicePosition4D {

  val list = List((Along[Position4D, First.type](First), First),
    (Along[Position4D, Second.type](Second), Second),
    (Along[Position4D, Third.type](Third), Third),
    (Along[Position4D, Fourth.type](Fourth), Fourth))
...
class TestOverPosition5D extends FlatSpec with Matchers with TestSlicePosition5D {

  val list = List((Over[Position5D, First.type](First), First),
    (Over[Position5D, Second.type](Second), Second),
    (Over[Position5D, Third.type](Third), Third),
    (Over[Position5D, Fourth.type](Fourth), Fourth),
...
class TestAlongPosition5D extends FlatSpec with Matchers with TestSlicePosition5D {

  val list = List((Along[Position5D, First.type](First), First),
    (Along[Position5D, Second.type](Second), Second),
    (Along[Position5D, Third.type](Third), Third),
    (Along[Position5D, Fourth.type](Fourth), Fourth),
