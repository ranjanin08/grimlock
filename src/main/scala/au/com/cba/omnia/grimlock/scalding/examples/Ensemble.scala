...
class Ensemble(args: Args) extends Job(args) {
  ...
  val schema = List(("entity.id", NominalSchema(StringCodex)),
    ("label", NominalSchema(LongCodex)),
    ("x", ContinuousSchema(DoubleCodex)),
    ("y", ContinuousSchema(DoubleCodex)),
    ("z", ContinuousSchema(DoubleCodex)))

  // Load the data (generated by ensemble.R); returns a 2D matrix (instance x feature);
  val data = loadText(s"${path}/exampleEnsemble.txt", Cell.parseTable(schema, separator = "|"))

  // Ensemble scripts to apply to the data.
  val scripts = List(s"${path}/gbm.R", s"${path}/lr.R", s"${path}/rf.R")

  // Weigh each model equally.
  val weights = ValuePipe(Map(Position1D(scripts(0)) -> Content(ContinuousSchema(DoubleCodex), 0.33),
    Position1D(scripts(1)) -> Content(ContinuousSchema(DoubleCodex), 0.33),
    Position1D(scripts(2)) -> Content(ContinuousSchema(DoubleCodex), 0.33)))
