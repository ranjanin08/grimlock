...
trait Schema {
   ...
   */
  def toShortString(separator: String): String = kind.name + separator + codex.name
}
...
object Schema {
  ...
  def fromString(codex: String, kind: String): Option[Schema] = {
    (kind, codex) match {
      case (Type.Continuous.name, DoubleCodex.name) => Some(ContinuousSchema[Codex.DoubleCodex]())
      case (Type.Continuous.name, LongCodex.name) => Some(ContinuousSchema[Codex.LongCodex]())
      case (Type.Discrete.name, LongCodex.name) => Some(DiscreteSchema[Codex.LongCodex]())
      case (Type.Nominal.name, StringCodex.name) => Some(NominalSchema[Codex.StringCodex]())
      case (Type.Nominal.name, DoubleCodex.name) => Some(NominalSchema[Codex.DoubleCodex]())
      case (Type.Nominal.name, LongCodex.name) => Some(NominalSchema[Codex.LongCodex]())
      case (Type.Ordinal.name, StringCodex.name) => Some(OrdinalSchema[Codex.StringCodex]())
      case (Type.Ordinal.name, DoubleCodex.name) => Some(OrdinalSchema[Codex.DoubleCodex]())
      case (Type.Ordinal.name, LongCodex.name) => Some(OrdinalSchema[Codex.LongCodex]())
      case (Type.Date.name, DateCodex.name) => Some(DateSchema[Codex.DateCodex]())
      case _ => None
    }
  }
}
 ...
 *       object.
 */
case class ContinuousSchema[C <: Codex] private (minimum: Option[C#T], maximum: Option[C#T])(implicit val codex: C,
...
object ContinuousSchema {
   ...
   */
  def apply[C <: Codex](minimum: C#T, maximum: C#T)(implicit codex: C, num: Numeric[C#T]): ContinuousSchema[C] = {
 ...
 *       object.
 */
case class DiscreteSchema[C <: Codex] private (minimum: Option[C#T], maximum: Option[C#T], step: Option[C#T])(
...
object DiscreteSchema {
   ...
   */
  def apply[C <: Codex](minimum: C#T, maximum: C#T, step: C#T)(implicit codex: C,
    int: Integral[C#T]): DiscreteSchema[C] = DiscreteSchema(Some(minimum), Some(maximum), Some(step))
}
 ...
 *       object.
 */
case class NominalSchema[C <: Codex] private (domain: Option[List[C#T]])(implicit val codex: C)
...
object NominalSchema {
   ...
   */
  def apply[C <: Codex](domain: List[C#T])(implicit codex: C): NominalSchema[C] = NominalSchema(Some(domain))
}
 ...
 *       object.
 */
case class OrdinalSchema[C <: Codex] private (domain: Option[List[C#T]])(implicit val codex: C)
  extends CategoricalSchema[C#T] {
...
object OrdinalSchema {
   ...
   * @param domain A list of values the variable can take on.
   */
  def apply[C <: Codex](domain: List[C#T])(implicit codex: C): OrdinalSchema[C] = OrdinalSchema(Some(domain))
}

/** Schema for date variables. */
case class DateSchema[C <: DateAndTimeCodex]()(implicit val codex: C) extends Schema {
  val kind = Type.Date

  def isValid[V <: Value](value: V): Boolean = !value.asDate.isEmpty

  override def toString(): String = "DateSchema[" + codex.toString + "]()"
}

/** Schema for event variables. */
case class EventSchema[C <: EventCodex]()(implicit val codex: C) extends Schema {
  val kind = Type.Event

  def isValid[V <: Value](value: V): Boolean = !value.asEvent.isEmpty

  override def toString(): String = "EventSchema[" + codex.toString + "]()"
}
