...
trait Matrix[P <: Position] extends Persist[Cell[P]] {
  ...
  def pairwise[D <: Dimension, T](slice: Slice[P, D], operators: T)(implicit ev1: PosDimDep[P, D],
    ev2: Operable[T], ev3: slice.S =!= Position0D): TypedPipe[Cell[slice.R#M]] = {
  ...
  def types[D <: Dimension](slice: Slice[P, D], specific: Boolean = false)(
    implicit ev1: PosDimDep[P, D], ev2: slice.S =!= Position0D): TypedPipe[(slice.S, Type)] = {
    data
   ...
   */
  def unique[D <: Dimension](slice: Slice[P, D])(
    implicit ev: slice.S =!= Position0D): TypedPipe[Cell[slice.S]] = {
