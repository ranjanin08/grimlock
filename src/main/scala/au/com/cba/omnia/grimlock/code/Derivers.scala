...
trait MovingAverage { self: Deriver =>
  protected def present[P <: Position, D <: Dimension](sel: Slice[P, D]#S,
    coord: Value, curr: Double, prev: Option[(Value, Double)]) = {
    val cell = (sel.append(coord),
      Content(ContinuousSchema[Codex.DoubleCodex](), curr))

    prev match {
      case None => Some(Left(cell))
      case Some((c, v)) => Some(Right(List((sel.append(c),
        Content(ContinuousSchema[Codex.DoubleCodex](), v)), cell)))
    }
  }

  protected def getDouble(con: Content): Double = {
    con.value.asDouble.getOrElse(Double.NaN)
  }
}

/**
 * Trait for computing moving average in batch mode; that is, keep the last
 * N values and compute the moving average from it.
 */
trait BatchMovingAverage extends Deriver with Initialise with MovingAverage {
  ...
  /**
   * Indicates if averages should be output when a full window isn't available
   * yet.
   */
  val all: Boolean
  ...
  def present[P <: Position, D <: Dimension](sel: Slice[P, D]#S,
    ...
    val out = (all || lst.size == window) match {
      case true => present(sel, crd, compute(lst), t._2)
      case false => None
    }
  ...
  private def getCurrent[P <: Position, D <: Dimension](rem: Slice[P, D]#R,
    con: Content): (Value, Double) = {
    (rem.get(dim), getDouble(con))
  }

  private def updateList[P <: Position, D <: Dimension](rem: Slice[P, D]#R,
    con: Content, lst: List[(Value, Double)]): List[(Value, Double)] = {
    (if (lst.size == window) { lst.drop(1) } else { lst }) :+
      getCurrent(rem, con)
  }
 ...
 * @param dim    The dimension in `rem` from which to get the coordinate to
 *               append to `sel`.
 * @param all    Indicates if averages should be output when a full window
 *               isn't available yet.
 */
case class SimpleMovingAverage(window: Int, dim: Dimension = First,
  all: Boolean = false) extends BatchMovingAverage {
  protected val idx = window - 1

  protected def compute(lst: List[(Value, Double)]): Double = {
    lst.foldLeft(0.0)((c, p) => p._2 + c) / lst.size
  }
}
 ...
 *
 * @param width Number of values before and after a given value to use when
 *              computing the moving average.
 * @param dim   The dimension in `rem` from which to get the coordinate to
 *              append to `sel`.
 */
case class CenteredMovingAverage(width: Int, dim: Dimension = First)
  extends BatchMovingAverage {
 ...
 *               isn't available yet.
 */
case class WeightedMovingAverage(window: Int, dim: Dimension = First,
  all: Boolean = false) extends BatchMovingAverage {
  protected val idx = window - 1

  protected def compute(lst: List[(Value, Double)]): Double = {
    val curr = lst.zipWithIndex.foldLeft((0.0, 0))((c, p) =>
      ((p._2 + 1) * p._1._2 + c._1, c._2 + p._2 + 1))
 ...
 *            append to `sel`.
 */
case class CumulativeMovingAverage(dim: Dimension = First)
  extends OnlineMovingAverage {
  protected def compute(curr: Double, t: T): Double = {
    (curr + t._2 * t._1) / (t._2 + 1)
  }
}
 ...
 *              append to `sel`.
 */
case class ExponentialMovingAverage(alpha: Double, dim: Dimension = First)
  extends OnlineMovingAverage {
  protected def compute(curr: Double, t: T): Double = {
    alpha * curr + (1 - alpha) * t._1
  }
}
